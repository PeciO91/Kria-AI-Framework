"""Detection-only Vitis AI structured pruning for YOLOv26.

The Vitis and PyTorch dependencies are imported only after CLI parsing so that
``python -m kria_ai yolov26 detection optimize --help`` also works outside the
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m kria_ai yolov26 detection optimize",
        description="Vitis AI 3.5 structured pruning for YOLOv26 detection",
    )
    parser.add_argument("--model", help="Detection model ID")
    parser.add_argument("--dataset", help="Detection dataset ID")
    parser.add_argument(
        "--method",
    ...
    parser.add_argument(
        "--lr", type=float, default=1e-3, help="Fine-tuning learning rate"
    )
    parser.add_argument(
        "--subset-len",
        "--subset",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Detection data-loader batch size",
    )
    ...
    parser.add_argument(
        "--device",
        default="auto",
        help="PyTorch device (auto, cpu, cuda, cuda:N)",
    )
    parser.add_argument(
        "--checkpoint",
        help="Local source YOLO checkpoint (defaults to the model registry entry)",
    )
    parser.add_argument(
        "--output-checkpoint",
...
def _install_vitis_deepcopy_workaround(torch_module: Any) -> None:
    ...
    Vitis 3.5 generates slim models with ``copy.deepcopy``. Its torch-function
    override can make ``Tensor.__deepcopy__`` fail on traced tensors, including
    one-step search candidates. Keep normal deepcopy behavior and only fall
    back to a clone for that specific failure.
    """
...
def detection_objective(model: Any, dataloader: Any) -> float:
    """Return negative E2E detection loss for Vitis' maximize-score API."""

    from kria_ai.yolov26.detection.evaluate import evaluate_loss

    return -float(evaluate_loss(model, dataloader))


def calibrate_batch_norm(model: Any, dataloader: Any) -> None:
    """Refresh BN statistics using the YOLO training/E2E forward path."""

    import torch

    from kria_ai.yolov26.detection.loss import forward_for_loss
...
def finetune_detection(
    ...
    *,
    ...
    """Fine-tune a sparse or slim model with the YOLOv26 E2E loss."""
    ...
    for epoch in range(epochs):
        ...
        for images, targets in dataloader:
            ...
            if not isinstance(loss, torch.Tensor):
                raise TypeError(
                    "YOLOv26 detection loss must reduce to a tensor, "
            ...
            batches += 1
        if not batches:
            raise RuntimeError("The detection fine-tuning loader produced no batches")
        scheduler.step()
        average_loss = total_loss / batches
        epoch_losses.append(average_loss)
        print(f"epoch={epoch + 1}/{epochs} detection_loss={average_loss:.6f}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        f"model={model_config.id} method={args.method} mode={args.mode} "
        f"device={device} excludes={len(excludes)}"
    )
    ...
    with _working_directory(output_dir):
        ...
        if args.mode in _MODES_WITH_PRUNING:
            ...
            if finetune_requested:
                training_losses = finetune_detection(
                    pruned_model,
                    train_loader,
                    epochs=args.epochs,
                    lr=args.lr,
            ...
            if args.method == "iterative":
                ...
                ) = _materialize_iterative_slim_model(
                    runner,
                    pruned_model,
                    ratio=args.ratio,
                    excludes=excludes,
    ...
    if final_model is not None:
        ...
        print(f"Optimized detection checkpoint: {output_checkpoint}")

    manifest = {
        "artifact_type": "yolov26_detection_optimizer",
        "model_id": model_config.id,
        "dataset_id": dataset_config.id,

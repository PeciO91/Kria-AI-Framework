...
try:
    ...
    try:
        from pytorch_nndct.apis import IterativePruningRunner
        HAS_PRUNER = True
    except ImportError:
        HAS_PRUNER = False
...
def prepare_model(m_cfg, d_cfg, device, prune_threshold=None):
    ...
    # 2a. Classification: adapt last layer to match checkpoint dimensions.
    # The checkpoint is the source of truth for the number of classes.
    if m_cfg.get('type') == 'classification':
        ...
        if os.path.exists(abs_weight_path):
            try:
                checkpoint = torch.load(abs_weight_path, map_location='cpu')
                ...
                fc_weight_key = f"{last_layer_name}.weight"
                if fc_weight_key in sd:
                    num_classes = sd[fc_weight_key].shape[0]
            except Exception as e:
                print(f"[WARN] Could not detect classes from checkpoint: {e}")

        # Fallback to dataset config
        if num_classes is None:
            num_classes = d_cfg.get('num_classes', 1000)
    ...
    if os.path.exists(abs_pruned_path) and prune_threshold is not None and HAS_PRUNER:
        ...
        # We always rebuild via IterativePruningRunner here because it can
        # consume the .spec generated by either method.
    ...
    elif prune_threshold is not None and not HAS_PRUNER:
        print(f"[WARN] prune_threshold={prune_threshold} given but pytorch_nndct not installed.")
